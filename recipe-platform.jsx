...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ingredient-amount {
    ...
    color: var(--terra); white-space: nowrap; min-width: 60px;
    font-weight: 700;
  }
  ...
  .pantry-header { margin-bottom: 2rem; }
  ...
  .planner-meal-type {
    ...
    text-transform: uppercase; color: var(--terra); letter-spacing: 0.08em;
    margin-bottom: 0.2rem;
  }
  .planner-meal-name {
    font-size: 0.78rem; font-weight: 500; color: var(--bark); line-height: 1.3;
  }
  .planner-cal {
    font-family: 'Space Mono', monospace; font-size: 0.65rem;
  ...
  .loading-state {
    ...
    justify-content: center; gap: 1.5rem; padding: 4rem 2rem;
    text-align: center;
  }
  .loading-spinner {
  ...
  .loading-label {
    font-family: 'Cormorant Garamond', serif;
    font-size: 1.3rem; color: var(--bark); font-weight: 300;
  }
  .loading-sublabel { font-size: 0.85rem; color: var(--smoke); }

  /* EMPTY */
  .empty-state {
    text-align: center; padding: 4rem 2rem; color: var(--smoke);
  }
  ...
  .btn-cancel:hover { border-color: var(--bark); color: var(--bark); }
  .btn-confirm {
    padding: 0.6rem 1.25rem; border-radius: 10px;
    background: var(--terra); color: white; border: none;
    cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; font-weight: 500;
    transition: all 0.2s;
  }
  .btn-confirm:hover:not(:disabled) { background: #b5541f; }
...
const IMPROVE_PROMPTS = [
  "Make it vegetarian","Reduce calories by 30%","Make it high protein",
  "Turn into meal prep","Make it keto","Add more vegetables",
  "Make it spicier","Reduce cooking time","Make it dairy-free",
  "Add a flavor twist"
];
...
// ─── Prompt Builder ─────────────────────────────────────────────────────────
function buildRecipePrompt({ ingredients, cuisine, diet, time, skill, calories }) {
  const ing = ingredients.length ? ingredients.join(", ") : "pantry staples";
  return `You are a world-class chef and culinary AI. Create a detailed, restaurant-quality recipe using these inputs.
...
OUTPUT FORMAT — respond ONLY with valid JSON, no markdown, no explanation:
{
  ...
  "description": "One enticing sentence (max 120 chars)",
  "time": "e.g. 35 min",
  "difficulty": "Easy|Intermediate|Advanced",
  "servings": 2,
  "ingredients": [
    { "amount": "2 tbsp", "name": "ingredient name" }
  ],
  "steps": [
    "Clear step 1",
    "Clear step 2"
...
function buildImprovePrompt(recipe, instruction) {
  return `You are a culinary AI. Modify this recipe per the instruction.

ORIGINAL RECIPE: ${JSON.stringify(recipe)}

INSTRUCTION: "${instruction}"

Apply the instruction intelligently. Return ONLY valid JSON in the exact same schema as the original. Keep what works, change what's needed.`;
}
    ...
    <div className="recipe-card" onClick={onClick}>
      ...
      <div className="recipe-card-body">
        ...
        <div className="recipe-card-meta">
          ...
          <span className="recipe-card-rating">★ {recipe.rating}</span>
        </div>
      </div>
    </div>
  );
...
function RecipeOutput({ recipe, onSave, onImprove, saved }) {
      ...
      <div className="recipe-header">
        ...
        <div className="recipe-meta">
          <div className="recipe-meta-item">
            <span className="recipe-meta-label">Time</span>
            <span className="recipe-meta-val">⏱ {recipe.time}</span>
          </div>
          <div className="recipe-meta-item">
            <span className="recipe-meta-label">Difficulty</span>
            <span className="recipe-meta-val">◆ {recipe.difficulty}</span>
          </div>
          <div className="recipe-meta-item">
            <span className="recipe-meta-label">Servings</span>
            <span className="recipe-meta-val">◎ {recipe.servings}</span>
          </div>
          <div className="recipe-meta-item">
            <span className="recipe-meta-label">Calories</span>
            <span className="recipe-meta-val">🔥 {recipe.nutrition?.calories}</span>
          </div>
        </div>
      </div>
...
function GeneratorPage({ onSave, savedIds }) {
  ...
  const generate = async () => {
    ...
    try {
      const prompt = buildRecipePrompt({ ingredients, cuisine, diet, time, skill, calories });
  ...
  const improve = async (instruction) => {
    if (!recipe || improving) return;
    setImproving(true);
    try {
      const prompt = buildImprovePrompt(recipe, instruction);
      const result = await callClaude(prompt);
      setRecipe(result);
    } catch(e) {
    ...
    <div className="page">
      <div className="hero">
        ...
        <h1 className="hero-title">
          Turn any ingredients<br/>into <em>extraordinary</em> meals
        </h1>
      ...
      <div className="gen-card">
        <div className="gen-card-title">
          <span style={{fontSize:"1.4rem"}}>◎</span> Your Ingredients
        </div>
        <div className="ingredient-input-row">
          <input
            className="ingredient-input"
            placeholder="Add an ingredient (e.g. chicken, garlic…)"
            value={input}
        ...
        <div style={{marginBottom:"0.75rem"}}>
          <span style={{fontSize:"0.78rem",color:"var(--smoke)",marginRight:"0.5rem",fontFamily:"Space Mono,monospace",textTransform:"uppercase",letterSpacing:"0.08em",fontSize:"0.65rem"}}>Quick add:</span>
          {QUICK.map(q => (
            <button key={q} onClick={() => setIngredients(prev => prev.includes(q) ? prev : [...prev, q])}
              style={{background:"none",border:"1px solid var(--border)",borderRadius:"100px",padding:"0.2rem 0.6rem",fontSize:"0.78rem",cursor:"pointer",marginRight:"0.35rem",marginBottom:"0.35rem",color:"var(--smoke)",fontFamily:"DM Sans,sans-serif",transition:"all 0.15s"}}
              onMouseOver={e=>e.target.style.color="var(--bark)"} onMouseOut={e=>e.target.style.color="var(--smoke)"}
            >{q}</button>
          ))}
        </div>
        <div className="chip-list">
          ...
          {ingredients.map(ing => (
            <span key={ing} className="chip">
              {ing}
              <button className="chip-x" onClick={() => setIngredients(prev => prev.filter(i=>i!==ing))}>×</button>
            </span>
          ))}
        </div>

        <div style={{borderTop:"1px solid var(--border)",paddingTop:"1.25rem",marginBottom:"1.25rem"}}>
          <div style={{fontSize:"0.78rem",fontFamily:"Space Mono,monospace",textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--smoke)",marginBottom:"0.75rem",fontSize:"0.65rem"}}>Preferences</div>
          <div className="filter-row">
            <select className="select-field" value={cuisine} onChange={e=>setCuisine(e.target.value)}>
              {CUISINES.map(c=><option key={c}>{c}</option>)}
            </select>
            <select className="select-field" value={diet} onChange={e=>setDiet(e.target.value)}>
              {DIETS.map(d=><option key={d}>{d}</option>)}
            </select>
            <select className="select-field" value={time} onChange={e=>setTime(e.target.value)}>
              {TIMES.map(t=><option key={t}>{t}</option>)}
            </select>
            <select className="select-field" value={skill} onChange={e=>setSkill(e.target.value)}>
              {SKILLS.map(s=><option key={s}>{s}</option>)}
            </select>
            <select className="select-field" value={calories} onChange={e=>setCalories(e.target.value)}>
              {CALORIE_OPTIONS.map(c=><option key={c}>{c}</option>)}
            </select>
          </div>
        </div>

        <button className="btn-generate" onClick={generate} disabled={loading || improving}>
          {loading ? <><div className="shimmer"/><span>Generating your recipe…</span></> :
           improving ? <><div className="shimmer"/><span>Improving recipe…</span></> :
           <><span style={{fontSize:"1.3rem"}}>✦</span><span>Generate Recipe with AI</span></>}
        </button>
...
function DiscoverPage({ onSave, savedIds }) {
    ...
    <div className="page">
      <div className="discover-page">
        <div className="discover-header">
          ...
          <p style={{color:"var(--smoke)",fontSize:"0.9rem"}}>Browse curated recipes from global cuisines, filtered by your preferences.</p>
        </div>
        ...
        <div className="recipe-grid">
          {shown.map(r => (
            <RecipeCard key={r.id} recipe={r} saved={savedIds.has(r.name)} onSave={() => onSave(r)} />
          ))}
        </div>
      </div>
    </div>
...
function PantryPage({ onGenerateFromPantry }) {
  const [items, setItems] = useState(PANTRY_ITEMS);
  const [newItem, setNewItem] = useState("");

  const add = () => {
    if (newItem.trim()) {
      setItems(p => [...p, { name: newItem.trim(), qty: "1 unit", status: "ok" }]);
      setNewItem("");
    }
  };
    ...
    <div className="page">
      <div className="pantry-page">
        <div className="pantry-header">
          ...
          <p style={{color:"var(--smoke)",fontSize:"0.9rem"}}>Track what you have. We'll suggest recipes that use your ingredients.</p>
        </div>

        <div style={{display:"flex",gap:"0.5rem",marginBottom:"1.5rem"}}>
          <input className="ingredient-input" placeholder="Add ingredient to pantry…"
        ...
        <div className="pantry-grid">
          <div className="pantry-card">
            ...
            <button className="pantry-suggest-btn" onClick={onGenerateFromPantry}>
          ...
          <div>
            <div className="pantry-card" style={{marginBottom:"1rem"}}>
              ...
              {[["Olive Oil","Running low"],["Parmesan","Running low"],["Heavy Cream","Recipe needs"],["Basil","Recipe needs"]].map(([item,reason])=>(
                <div key={item} className="pantry-item">
                  <div className="pantry-item-name">
                    <span className="pantry-item-dot low"/>
                    {item}
                  </div>
            ...
            <div className="pantry-card">
              ...
              <p style={{fontSize:"0.85rem",color:"var(--smoke)",marginBottom:"0.75rem"}}>Based on your current pantry:</p>
    ...
    <div className="page">
      <div className="planner-page">
        <div className="pantry-header">
          ...
          <p style={{color:"var(--smoke)",fontSize:"0.9rem"}}>Personalized weekly plan based on your nutrition goals and preferences.</p>
        </div>
        ...
        <div style={{display:"flex",gap:"1rem",marginBottom:"1.5rem",flexWrap:"wrap"}}>
          ...
          <button style={{padding:"0.75rem 1.25rem",borderRadius:"12px",background:"var(--terra)",color:"white",border:"none",cursor:"pointer",fontFamily:"DM Sans,sans-serif",fontSize:"0.9rem",fontWeight:500,marginLeft:"auto"}}>
            ✦ Regenerate Plan
          </button>
        </div>
        ...
        <div className="planner-grid">
          {days.map(day => (
            <div key={day} className="planner-day">
              ...
              {[["B","b"],["L","l"],["D","d"]].map(([type,key])=>(
                <div key={type} className="planner-meal">
                  <div className="planner-meal-type">{type==="B"?"Breakfast":type==="L"?"Lunch":"Dinner"}</div>
...
export default function App() {
  ...
  const generateFromPantry = () => {
    setTab("generate");
    addToast("Loading pantry ingredients…", "◎");
  };
      ...
      <div className="app">
        <nav className="nav">
          <div className="nav-logo">
            <span style={{fontSize:"1.2rem"}}>◈</span> Culi<span>na</span>
          </div>
          <div className="nav-tabs">
            {[
              ...
              { id:"saved", label:"Saved", badge: saved.length || null },
            ].map(t => (
        ...
        {tab === "pantry" && <PantryPage onGenerateFromPantry={generateFromPantry} />}
        {tab === "planner" && <PlannerPage />}
        {tab === "saved" && <SavedPage saved={saved} onRemove={handleRemove} />}
